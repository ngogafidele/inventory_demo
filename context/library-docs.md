...
Project-specific usage notes for third-party libraries in Demo Inventory System.
...
- `proxy.ts` refreshes JWT sessions, clears expired access, records lapsed logouts, carries strict idle state, and redirects admin users to `/verify` for sensitive reporting routes.
- Server pages under `app/(dashboard)` should authenticate and fetch store-scoped data on the server.
- Route handlers under `app/api` are the mutation/query/PDF boundary.
- `next.config.ts` sets `serverExternalPackages: ["pdfkit"]` and currently ignores TypeScript build errors.
...
- Use `"use client"` only for managers and primitives that need state, effects, browser APIs, router refresh, charts, or event handlers.
...
- Existing code uses semantic Tailwind palettes for financial/status tints. Match local status patterns instead of inventing a new palette.

---

## Recharts

Version: `recharts@3.9.2`

Usage:

- Report charts live in `components/reports/report-charts.tsx`.
- Product movement charts live in `components/products/stock-movement-charts.tsx`.
- Prefer CSS variable colors from `--chart-*` and `--viz-*`.
- Keep charts responsive with `ResponsiveContainer` and preserve table/detail fallbacks where present.
...
- Define indexes in schema files for store-scoped lookup and uniqueness.
...
- Do not mutate or hard-delete older `BalanceSheetItem` versions.
...
- Session payloads include role, allowed stores, active store, login log id, idle timestamp, strict flag, and optional reauth timestamp.
...
- Current image assets are `public/images/logo.png`, `public/images/stamp.png`, and `public/images/auth-background.webp`.
